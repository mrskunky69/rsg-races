...
function DeleteCheckpoint()
    ...
    if RaceData.ClosestCheckpoint ~= 0 then
        ...
        if ClosestCheckpoint then
            ...
            RSGCore.Functions.Notify(Lang:t("error.slow_down"), 'error')
        end
    else
        RSGCore.Functions.Notify(Lang:t("error.slow_down"), 'error')
    end
...
function SaveRace()
    ...
    TriggerServerEvent('rsg-racing:server:SaveRace', CreatorData)
    Lang:t("error.slow_down")
    RSGCore.Functions.Notify(Lang:t("success.race_saved")..'('..CreatorData.RaceName..')', 'success')
...
function AddCheckpoint()
    ...
    local Offset = {
        left = {
            x = (GetOffsetFromEntityInWorldCoords(PlayerPed, -CreatorData.TireDistance, 0.0, 0.0)).x,
            y = (GetOffsetFromEntityInWorldCoords(PlayerPed, -CreatorData.TireDistance, 0.0, 0.0)).y,
            z = (GetOffsetFromEntityInWorldCoords(PlayerPed, -CreatorData.TireDistance, 0.0, 0.0)).z,
        },
        right = {
            x = (GetOffsetFromEntityInWorldCoords(PlayerPed, CreatorData.TireDistance, 0.0, 0.0)).x,
            y = (GetOffsetFromEntityInWorldCoords(PlayerPed, CreatorData.TireDistance, 0.0, 0.0)).y,
...
function CreateCheckpointBlip(coords, id)
    ...
    SetBlipSprite(race, `blip_proc_home`, true)
    SetBlipScale(race, 0.1)
...
function CreatorLoop()
    CreateThread(function()
        while RaceData.InCreator do
            ...
            if PlayerVeh ~= 0 then
                if IsControlJustPressed(1, 0xB03A913B) then -- Key 7 (Add checkpoint)
                    AddCheckpoint()
                end

                if IsControlJustPressed(1, 0x42385422) then -- Key 8 (Cancel editor)
                    if not CreatorData.ConfirmDelete then
                        CreatorData.ConfirmDelete = true
                        RSGCore.Functions.Notify(Lang:t("error.editor_confirm"), 'error')
                    else
                        DeleteCreatorCheckpoints()
                        RaceData.InCreator = false
                        CreatorData.RaceName = nil
                        CreatorData.Checkpoints = {}
                        RSGCore.Functions.Notify(Lang:t("error.editor_canceled"), 'error')
                        CreatorData.ConfirmDelete = false
                    end
                end

                if IsControlJustPressed(1, 0x05CA7C52) then -- J (Save race)
                    if CreatorData.Checkpoints and #CreatorData.Checkpoints >= Config.MinimumCheckpoints then
                        SaveRace()
                    else
                        RSGCore.Functions.Notify(Lang:t("error.not_enough_checkpoints")..'('..Config.MinimumCheckpoints..')', 'error')
                    end
                end

                if IsControlJustPressed(1, 0xA65EBAB4) then -- LEFT ARROW (Increase tire distance)
                    if CreatorData.TireDistance < Config.MaxTireDistance then
                        CreatorData.TireDistance = CreatorData.TireDistance + 1.0
                    else
                        RSGCore.Functions.Notify(Lang:t("error.max_tire_distance")..Config.MaxTireDistance)
                    end
                end

                if IsControlJustPressed(1, 0xDEB34313) then -- RIGHT ARROW (Decrease tire distance)
                    if CreatorData.TireDistance > Config.MinTireDistance then
                        CreatorData.TireDistance = CreatorData.TireDistance - 1.0
                    else
                        RSGCore.Functions.Notify(Lang:t("error.min_tire_distance")..Config.MinTireDistance)
                    end
                end
            else
                local coords = GetEntityCoords(PlayerPedId())
                DrawText3D(coords.x, coords.y, coords.z, Lang:t("text.get_in_vehicle"))
            end
...
function FinishRace()
    TriggerServerEvent('rsg-racing:server:FinishPlayer', CurrentRaceData, CurrentRaceData.TotalTime, CurrentRaceData.TotalLaps, CurrentRaceData.BestLap)
    RSGCore.Functions.Notify(Lang:t("success.race_finished")..SecondsToClock(CurrentRaceData.TotalTime), 'success')
    if CurrentRaceData.BestLap ~= 0 then
        RSGCore.Functions.Notify(Lang:t("success.race_best_lap")..SecondsToClock(CurrentRaceData.BestLap), 'success')
    end
                ...
                if CheckpointDistance < MaxDistance then
                    if CurrentRaceData.TotalLaps == 0 then
                        if CurrentRaceData.CurrentCheckpoint + 1 < #CurrentRaceData.Checkpoints then
                            ...
                            TriggerServerEvent('rsg-racing:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, false)
                            DoPilePfx()
                            ...
                            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                            CurrentRaceData.CurrentCheckpoint = CurrentRaceData.CurrentCheckpoint + 1
                            TriggerServerEvent('rsg-racing:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, true)
                            FinishRace()
                        end
                    else
                        if CurrentRaceData.CurrentCheckpoint + 1 > #CurrentRaceData.Checkpoints then
    ...
    while true do
        if RaceData.InCreator then
            ...
            if PlayerVeh ~= 0 then
                ...
                DrawText3D(Offset.left.x, Offset.left.y, Offset.left.z, Lang:t("text.checkpoint_left"))
                DrawText3D(Offset.right.x, Offset.right.y, Offset.right.z, Lang:t("text.checkpoint_right"))
            end
        end
        Wait(0)
    end
...
RegisterNetEvent('rsg-racing:server:ReadyJoinRace', function(RaceData)
    RaceData.RacerName = RaceData.SetupRacerName
    TriggerServerEvent('rsg-racing:server:JoinRace', RaceData)
end)

RegisterNetEvent('rsg-racing:client:StartRaceEditor', function(RaceName, RacerName)
    if not RaceData.InCreator then
        ...
        RSGCore.Functions.Notify(Lang:t("error.already_making_race"), 'error')
    end
end)

RegisterNetEvent('rsg-racing:client:UpdateRaceRacerData', function(RaceId, RaceData)
    if (CurrentRaceData.RaceId ~= nil) and CurrentRaceData.RaceId == RaceId then
        CurrentRaceData.Racers = RaceData.Racers
    end
end)

RegisterNetEvent('rsg-racing:client:JoinRace', function(Data, Laps, RacerName)
    if not RaceData.InRace then
        Data.RacerName = RacerName
        RaceData.InRace = true
        SetupRace(Data, Laps)
        RSGCore.Functions.Notify(Lang:t("primary.race_joined"))
        TriggerServerEvent('rsg-racing:server:UpdateRaceState', CurrentRaceData.RaceId, false, true)
    else
        RSGCore.Functions.Notify(Lang:t("error.already_in_race"), 'error')
    end
end)

RegisterNetEvent('rsg-racing:client:UpdateRaceRacers', function(RaceId, Racers)
    if CurrentRaceData.RaceId == RaceId then
        CurrentRaceData.Racers = Racers
    end
end)

RegisterNetEvent('rsg-racing:client:LeaveRace', function(data)
    DeleteCurrentRaceCheckpoints()
    FreezeEntityPosition(GetVehiclePedIsIn(PlayerPedId(), false), false)
end)

RegisterNetEvent('rsg-racing:client:RaceCountdown', function()
    TriggerServerEvent('rsg-racing:server:UpdateRaceState', CurrentRaceData.RaceId, true, false)
    if CurrentRaceData.RaceId ~= nil then
        while Countdown ~= 0 do
            if CurrentRaceData.RaceName ~= nil then
                if Countdown == 10 then
                    RSGCore.Functions.Notify(Lang:t("primary.race_will_start"), 'primary', 2500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                elseif Countdown <= 5 then
                    RSGCore.Functions.Notify(Countdown, 'error', 500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                end
        ...
        if CurrentRaceData.RaceName ~= nil then
            RSGCore.Functions.Notify(Lang:t("success.race_go"), 'success', 1000)
            SetBlipScale(CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].blip, 1.0)
...
RegisterNetEvent('rsg-racing:client:PlayerFinish', function(RaceId, Place, RacerName)
    if CurrentRaceData.RaceId ~= nil then
        if CurrentRaceData.RaceId == RaceId then
            RSGCore.Functions.Notify(RacerName..Lang:t("primary.racer_finished_place")..Place, 'primary', 3500)
        end
    end
end)

RegisterNetEvent('rsg-racing:client:WaitingDistanceCheck', function()
    Wait(1000)
    CreateThread(function()
        while true do
                ...
                if CurrentRaceData.Checkpoints[1] ~= nil then
                    ...
                    if dist > 115.0 then
                        if ToFarCountdown ~= 0 then
                            ...
                            RSGCore.Functions.Notify(Lang:t("error.return_to_start")..ToFarCountdown..'s', 'error', 500)
                        else
                            TriggerServerEvent('rsg-racing:server:LeaveRace', CurrentRaceData)
                            ToFarCountdown = 10
                            break
                        end
                        Wait(1000)
...
RegisterNetEvent("rsg-racing:Client:OpenMainMenu", function(data)
    ...
    exports['rsg-menu']:openMenu({
        ...
        },
        {
            ...
            params = {
                event = "rsg-racing:Client:CurrentRaceMenu",
        ...
        },
        {
            ...
            params = {
                event = "rsg-racing:Client:AvailableRacesMenu",
                args = { type = type, name = name }
            }
        },
        {
            ...
            params = {
                event = "rsg-racing:Client:RaceRecordsMenu",
                args = { type = type, name = name }
            }
        },
        {
            ...
            params = {
                event = "rsg-racing:Client:SetupRaceMenu",
                args = { type = type, name = name }
            }
        },
        {
            ...
            params = {
                event = "rsg-racing:Client:CreateRaceMenu",
                args = { type = type, name = name }
            }
        },
        {
...
RegisterNetEvent("rsg-racing:Client:CurrentRaceMenu", function(data)
    ...
    exports['rsg-menu']:openMenu({
        ...
        },
        {
            ...
            txt = "",
            disabled = (not (CurrentRaceData.OrganizerCID == RSGCore.Functions.GetPlayerData().citizenid) or CurrentRaceData.Started),
            params = {
                isServer = true,
                event = "rsg-racing:server:StartRace",
        ...
        },
        {
            ...
            params = {
                isServer = true,
                event = "rsg-racing:server:LeaveRace",
        ...
        },
        {
            ...
            params = {
                event = "rsg-racing:Client:OpenMainMenu",
...
RegisterNetEvent("rsg-racing:Client:AvailableRacesMenu", function(data)
    RSGCore.Functions.TriggerCallback('rsg-racing:server:GetRaces', function(Races)
        local menu = {
            {
                header = Lang:t("menu.available_races"),
                isMenuHeader = true
        ...
        for _,race in ipairs(Races) do
            ...
            menu[#menu+1] = {
                ...
                params = {
                    isServer = true,
                    event = "rsg-racing:server:JoinRace",
                    args = race
                }
            }
        end
        ...
        if #menu == 2 then
            RSGCore.Functions.Notify(Lang:t("primary.no_pending_races"))
            TriggerEvent('rsg-racing:Client:OpenMainMenu', { type = data.type, name = data.name })
            return
        end
...
RegisterNetEvent("rsg-racing:Client:RaceRecordsMenu", function(data)
    RSGCore.Functions.TriggerCallback('rsg-racing:server:GetRacingLeaderboards', function(Races)
        local menu = {
            {
                header = Lang:t("menu.race_records"),
                isMenuHeader = true
        ...
        if #menu == 2 then
            RSGCore.Functions.Notify(Lang:t("primary.no_races_exist"))
            TriggerEvent('rsg-racing:Client:OpenMainMenu', { type = data.type, name = data.name })
            return
        end
...
RegisterNetEvent("rsg-racing:Client:SetupRaceMenu", function(data)
    RSGCore.Functions.TriggerCallback('rsg-racing:server:GetListedRaces', function(Races)
        ...
        if #tracks == 1 then
            RSGCore.Functions.Notify(Lang:t("primary.no_available_tracks"))
            TriggerEvent('rsg-racing:Client:OpenMainMenu', { type = data.type, name = data.name })
            return
        end
        ...
        TriggerServerEvent('rsg-racing:server:SetupRace', dialog.track, tonumber(dialog.laps), data.name)
    end)
end)

RegisterNetEvent("rsg-racing:Client:CreateRaceMenu", function(data)
    local dialog = exports['rsg-input']:ShowInput({
    ...
    if #dialog.trackname < Config.MinTrackNameLength then
        RSGCore.Functions.Notify(Lang:t("error.name_too_short"), "error")
        TriggerEvent("rsg-racing:Client:CreateRaceMenu", { type = data.type, name = data.name })
        return
    end

    if #dialog.trackname > Config.MaxTrackNameLength then
        RSGCore.Functions.Notify(Lang:t("error.name_too_long"), "error")
        TriggerEvent("rsg-racing:Client:CreateRaceMenu", { type = data.type, name = data.name })
        return
    end

    RSGCore.Functions.TriggerCallback('rsg-racing:server:IsAuthorizedToCreateRaces', function(IsAuthorized, NameAvailable)
        if not IsAuthorized then return end
        if not NameAvailable then
            RSGCore.Functions.Notify(Lang:t("error.race_name_exists"), "error")
            TriggerEvent("rsg-racing:Client:CreateRaceMenu", { type = data.type, name = data.name })
            return
        end

        TriggerServerEvent('rsg-racing:server:CreateLapRace', dialog.trackname, data.name)
